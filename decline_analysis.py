...
neg_rates = []
log("  % негатива по диапазонам (playtime_forever):")
...
early_at_review = []
for label, minutes in [("<30min at review", 30), ("<60min at review", 60)]:
    subset = [r for r in reviews if float(r['playtime_at_review_minutes']) < minutes]
    if subset:
        neg_n = sum(1 for r in subset if r['voted_up'] == '0')
        pct = neg_n / len(subset) * 100
        early_at_review.append((label, len(subset), pct))
        log(f"  Негатив при {label}: {neg_n}/{len(subset)} = {pct:.1f}%")

early_forever_n = sum(1 for r in reviews if float(r['playtime_forever_minutes']) < 30)
early_forever_neg = sum(1 for r in reviews if r['voted_up'] == '0' and float(r['playtime_forever_minutes']) < 30)
early_forever_pct = early_forever_neg / early_forever_n * 100 if early_forever_n else 0
...
hypotheses = [
    ...
    ("H2", "Игроки уходят из-за проблем в первые часы",
     f"Негатив при плейтайме <30 мин: {early_forever_pct:.1f}% (n={early_forever_n}); <1ч: в среднем {first_hour_neg:.1f}%",
     "ДА" if early_forever_pct > 35 else "ЧАСТИЧНО", "Высокая"),
    ...
    ("H7", "Высокие системные требования отсекают аудиторию",
     "Топ GPU — RTX 5070 Ti (5.5% среди 401 с HW-данными), Windows 11 — 78.2%",
     "ЧАСТИЧНО", "Средняя"),
...
neg_at_review_30 = next((pct for lbl, n, pct in early_at_review if lbl == "<30min at review"), 0)
neg_at_review_n = next((n for lbl, n, pct in early_at_review if lbl == "<30min at review"), 0)

recommendations = [
    ("P1", "Улучшить первые 30 мин (туториал, вовлечение)",
     f"Негатив при плейтайме <30 мин: {early_forever_pct:.1f}% (n={early_forever_n}); при отзыве <30 мин: {neg_at_review_30:.1f}% (n={neg_at_review_n}). Интерактивный туториал, матч с другом сразу, быстрый старт."),
...
lines.append("-"*40)
lines.append(f"  Негатив при плейтайме <30 мин: {early_forever_pct:.1f}% (n={early_forever_n})")
lines.append(f"  Негатив при отзыве <30 мин: {neg_at_review_30:.1f}% (n={neg_at_review_n})")
